...
namespace Boxfusion.Smartgov.Epm.Web.Host.Controllers
{
    public class HomeController : SheshaControllerBase
    {
        ...
        public IActionResult TestNh()
        {
            try
            {
                ...
                var user = currentSession.Get<User>((Int64)1);
                var tenant = currentSession.Get<Shesha.MultiTenancy.Tenant>((int)1);
                var role = currentSession.Get<Role>((int)1);
