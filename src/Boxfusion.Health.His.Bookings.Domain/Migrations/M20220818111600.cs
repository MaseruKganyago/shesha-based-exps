...
namespace Boxfusion.Health.His.Bookings.Migrations
{
    ...
    /// 
    /// </summary>
    [Migration(20220818111600)]
    public class M20220818111600 : Migration
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        {
            Execute.Sql(@"/****** Object:  View [dbo].[vw_His_FlattenedFacilityAppointments]    Script Date: 2022/08/18 09:48:41 ******/
SET ANSI_NULLS ON
GO
...
ALTER   VIEW [dbo].[vw_His_FlattenedFacilityAppointments]
			AS
				SELECT 
					app.Id
					,app.RefNumber
					...
					, practitioner.FullName [PractitionerName]
					,app.PatientId
					...
					,per.[Fhir_IdentificationTypeLkp] PatientIdentificationTypeLkp
				...
				FROM Fhir_Appointments app
					...
					LEFT JOIN Core_Persons practitioner on practitioner.Id = app.PractitionerId
				WHERE
					per.Frwk_Discriminator in ('His.HisPatient', 'Fhir.CdmPatient')
