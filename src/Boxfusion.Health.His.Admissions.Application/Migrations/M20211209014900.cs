...
namespace Boxfusion.Health.HealthCommon.Core.Migrations
{
    ...
    /// 
    /// </summary>
    [Migration(20211209014900)]
    public class M20211209014900 : Migration
    {
        /// <summary>
        /// 
        /// </summary>
...
CREATE OR ALTER     PROCEDURE [dbo].[GetWardCensusDailyStats] 
		...
		DECLARE @TotalNumberOfAdmissions INT;
			...
			SELECT COUNT(*)
				...
				AND IsDeleted = 0
				AND CAST(StartDateTime AS DATE) = DATEADD(day, -1, CAST(@ReportDate AS date))
			...
			SELECT COUNT(*)  FROM Fhir_Encounters  
				...
				and (CAST(StartDateTime AS DATE) = DATEADD(day, 0, CAST(@ReportDate AS date)) 
				or CAST(StartDateTime AS DATE) = DATEADD(day, -1, CAST(@ReportDate AS date)) )
				and His_AdmissionStatusLkp = 1 /*Admitted*/
			)
